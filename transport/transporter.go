...
// a context via [NewContext] so that downstream handlers can inspect the
...
// NewContext returns a copy of ctx with the given [Transporter] attached.
func NewContext(ctx context.Context, tr Transporter) context.Context {
	return context.WithValue(ctx, transporterKey{}, tr)
}

// FromContext extracts the [Transporter] from ctx, if present.
func FromContext(ctx context.Context) (Transporter, bool) {
